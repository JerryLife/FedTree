...
//        n_features_ = words.size() - 2;
    ...
    ifs.clear();
    ifs.seekg(0);   // reset ifs to the beginning since the csv file does not have header by default.
...
#pragma omp parallel num_threads(nthread)
        {
            ...
            while(line_begin != block_end) {
                ...
                std::ptrdiff_t advanced = ignore_comment_and_blank(p, line_end);
                p += advanced;
//                int r = parse_pair<float_type, float_type>(p, line_end, &q, id, temp_);   // read id
