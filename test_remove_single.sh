...
for ratio in 0.001 0.01; do
  if [ $ratio = "0.01" ]; then
    ...
    taskset -c $cpus ./main conf/"$dataset"_1e-02.conf data=./data/"$dataset".train.remain_1e-02 remove_ratio="$ratio" n_trees=$n_trees performe_remove=false \
    ...
    if [ $ratio = "0.001" ]; then
      taskset -c $cpus ./main conf/"$dataset"_1e-03.conf data=./data/"$dataset".train remove_ratio="$ratio" n_trees=$n_trees \
          ...
          $outdir/"$dataset"_deltaboost_"$ratio".out
      taskset -c $cpus ./main conf/"$dataset"_1e-03.conf data=./data/"$dataset".train.remain_1e-03 remove_ratio="$ratio" n_trees=$n_trees performe_remove=false \
