...
for dataset in codrna cadata covtype gisette msd; do
  for ratio in 0.01 0.001; do
    if [ $ratio = "0.01" ]; then
      ...
      taskset -c $cpus ./main conf/"$dataset".conf data=./data/"$dataset".train.remain_1e-02 remove_ratio="$ratio" n_trees=$n_trees \
      ...
      taskset -c $cpus ./main conf/"$dataset".conf save_model_name="$dataset"_tree"$n_trees"_gbdt data=./data/"$dataset".train enable_delta=false n_trees=$n_trees > \
      ...
      if [ $ratio = "0.001" ]; then
        taskset -c $cpus ./main conf/"$dataset".conf data=./data/"$dataset".train remove_ratio="$ratio" n_trees=$n_trees \
            ...
            $outdir/"$dataset"_deltaboost_"$ratio".out &
          taskset -c $cpus ./main conf/"$dataset".conf data=./data/"$dataset".train.remain_1e-03 remove_ratio="$ratio" n_trees=$n_trees \
